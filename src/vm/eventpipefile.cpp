...
)
{
    ...
    if (!m_pMetadataBlock->WriteEvent(instance, 0, 0, 0, TRUE))
    {
        ...
        bool result = m_pMetadataBlock->WriteEvent(instance, 0, 0, 0, TRUE);
        _ASSERTE(result == true); // we should never fail to add event to a clear block (if we do the max size is too small)
    }

    if (pMetadataBlobV2 != nullptr && m_format >= EventPipeSerializationFormat::NetTraceV4)
    {
        m_pMetadataBlockV2->WriteMetadataBlob(metadataId, providerName, pMetadataBlobV2, metadataBlobLengthV2);
    }
}
