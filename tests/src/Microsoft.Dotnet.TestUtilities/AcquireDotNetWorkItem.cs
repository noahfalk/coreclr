...
namespace Microsoft.Dotnet.TestUtilities
{
    ...
    public class AcquireDotNetWorkItem
    {
        ...
        {
            return Download(sdkVersion, arch, localDotNetDirPath, false);
        }
        ...
        public static string GetCompatibleDefaultSDKVersion(string targetFrameworkMoniker)
        {
        ...
        async public Task Run(ITestOutputHelper output)
        {
            using (var indent = new IndentedTestOutputHelper("Acquiring DotNet", output))
            {
                ...
                else if (LocalDotNetZipPath != null)
                {
                    await FileTasks.Unzip(LocalDotNetZipPath, LocalDotNetZipExpandDirPath, indent);
                }
